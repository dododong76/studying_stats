<script>
    ...
    let ballCount = $derived.by(() => Math.max(30, Math.min(500, Number.parseInt(ballCountInput, 10) || 100)));
    ...
    function loop(ts) {
        ...
        for (let i = 0; i < activeBalls.length; i += 1) {
            ...
            if (b.step < rows && b.y >= b.targetY - 0.7) {
                ...
                b.step += 1;
                if (goRight) b.path += 1;
                // 핀 접촉 시 상향 + 수평 반발을 주어 자연스러운 튕김 아크를 만든다.
                b.vy = -0.5;
